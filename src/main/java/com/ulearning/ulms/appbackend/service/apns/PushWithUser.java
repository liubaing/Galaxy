...
public class PushWithUser extends AbstractPush {
	...
	public void pushAndroidNotifications(Apns apns) {
		jPushClient.sendNotificationWithAlias(apns.getId().getInc(), USER_KEY + apns.getRecipient(), apns.getMsgContent().getTitle(), apns.getMsgContent().getMessage());
	}
	...
	public void pushAppleNotifications(Apns apns) {
		...
		User user = userRepository.findOne(apns.getRecipient());
