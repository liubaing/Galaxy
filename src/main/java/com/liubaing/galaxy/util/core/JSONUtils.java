...
import com.liubaing.galaxy.util.log.LogUtils;
 /**
  * 类说明：包含操作 {@code JSON} 数据的常用方法的工具类。
  * @author heshuai
  *  <i>refer OpenSource</i>
 ...
 public class JSONUtils {
     ...
     /** 默认的 {@code JSON} 日期/时间字段的格式化模式。{@code yyyy-MM-dd HH:mm:ss} */
     ...
     public static String toJson(Object target, Type targetType, boolean isSerializeNulls, Double version,
         ...
         if (ObjectUtils.isEmpty(datePattern)) datePattern = DEFAULT_DATE_PATTERN;
     ...
     /**
      * 将给定的 {@code JSON} 字符串转换成指定的类型对象。<strong>此方法可以用来转换集合类。</strong>
      * 
      * @param <T> 要转换的目标类型。
     ...
     public static <T> T fromJson(String json, TypeToken<T> token) {
    	 if (ObjectUtils.isEmpty(json)) throw new IllegalArgumentException("json字符串不能为空");
         GsonBuilder builder = new GsonBuilder();
         Gson gson = builder.setDateFormat(DEFAULT_DATE_PATTERN).create();
         try {
             return gson.fromJson(json, token.getType());
         } catch (Exception ex) {
        	 LogUtils.doLog(json + " 无法转换为 " + token.getRawType().getName() + " 对象!", ex);
        	 throw new IllegalArgumentException(ex);
         }
     }
     
     /**
      * 将给定的 {@code JSON} 字符串转换成指定的类型对象。<strong>此方法通常用来转换普通的JavaBean。</strong>
      * 
      * @param <T> 要转换的目标类型。
      * @param json 给定的 {@code JSON} 字符串。
      * @param clazz 要转换的目标类。
     ...
     public static <T> T fromJson(String json, Class<T> clazz) {
         ...
         try {
        	 ...
        	 LogUtils.doLog(json + " 无法转换为 " + clazz.getName() + " 对象!", ex);
        	 throw new IllegalArgumentException(ex);
         }
     }
}
